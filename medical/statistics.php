...
require_login('medical', 'medical/statistics.php');
$cfg = base_config();
...
<head>
  ...
  <title>Medical Statistics - <?= e($cfg['app_name']) ?></title>
...
<body class="bg-light">
      ...
      <div class="row align-items-center">
        <div class="col-12 col-md">
          <div class="d-flex align-items-center gap-3">
            ...
            <div class="overflow-hidden">
              <div class="brand h5 mb-0 text-white fw-bold lh-1">Medical Statistics</div>
        ...
        <div class="col-12 col-md-auto mt-2 mt-md-0">
          <div class="d-flex align-items-center gap-2 justify-content-md-end">
            ...
            <a href="<?= url('/medical/index.php') ?>" class="btn btn-light btn-sm fw-bold px-3" style="border-radius: 8px;">Dashboard</a>
  ...
  <main class="container py-4 py-md-5">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-end gap-2 mb-3">
      <div>
        ...
        <div class="text-muted small">Aggregated from latest medical assessment per patient</div>
      </div>
      ...
      <div class="card-body p-3 p-md-4">
        <form method="get" class="row g-2 align-items-end">
          ...
          <div class="col-12 d-flex gap-2 justify-content-end mt-2">
            <a href="<?= url('/medical/statistics.php') ?>" class="btn btn-outline-secondary">Reset</a>
      ...
      <div class="col-6 col-lg-3">
        <div class="card border-0 shadow-sm" style="border-radius: 16px;">
          <div class="card-body">
            ...
            <div class="h4 fw-bold mb-0" id="kpiMedicalChecked">-</div>
          </div>
        </div>
      </div>
      <div class="col-6 col-lg-3">
        <div class="card border-0 shadow-sm" style="border-radius: 16px;">
          <div class="card-body">
            <div class="text-muted small">Medical pending</div>
            <div class="h4 fw-bold mb-0" id="kpiMedicalPending">-</div>
          </div>
        </div>
      </div>
      <div class="col-6 col-lg-3">
        <div class="card border-0 shadow-sm" style="border-radius: 16px;">
          <div class="card-body">
            <div class="text-muted small">With assessment</div>
            <div class="h4 fw-bold mb-0" id="kpiWithAssessment">-</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="row g-3">
      <div class="col-12 col-lg-6">
        <div class="card shadow-sm border-0" style="border-radius: 16px; overflow: hidden;">
          <div class="card-header bg-white border-0 py-3">
            <div class="fw-bold">BMI category (latest assessment)</div>
          </div>
          <div class="card-body">
      ...
      <div class="col-12 col-lg-6">
        <div class="card shadow-sm border-0" style="border-radius: 16px; overflow: hidden;">
          <div class="card-header bg-white border-0 py-3">
            <div class="fw-bold">Blood pressure category</div>
            <div class="text-muted small">Computed from latest BP values</div>
          </div>
          <div class="card-body">
            <div style="height: 260px;">
              <canvas id="chartBp"></canvas>
            </div>
            <div class="mt-3" id="tblBp"></div>
          </div>
        </div>
      </div>
      ...
      <div class="col-12 col-lg-6">
        <div class="card shadow-sm border-0" style="border-radius: 16px; overflow: hidden;">
          <div class="card-header bg-white border-0 py-3">
            <div class="fw-bold">Stress level</div>
          </div>
          <div class="card-body">
            <div style="height: 260px;">
              <canvas id="chartStress"></canvas>
            </div>
            <div class="mt-3" id="tblStress"></div>
          </div>
        </div>
      </div>
      <div class="col-12 col-lg-6">
        <div class="card shadow-sm border-0" style="border-radius: 16px; overflow: hidden;">
          <div class="card-header bg-white border-0 py-3">
            <div class="fw-bold">Coping level</div>
          </div>
          <div class="card-body">
            <div style="height: 260px;">
              <canvas id="chartCoping"></canvas>
            </div>
            <div class="mt-3" id="tblCoping"></div>
          </div>
        </div>
      </div>
        ...
        <div class="card shadow-sm border-0" style="border-radius: 16px; overflow: hidden;">
          ...
          <div class="card-body">
            <div style="height: 320px;">
              <canvas id="chartTopSchools"></canvas>
            </div>
  ...
  <script>
    (function(){
      var chartBmi = null;
      var chartBp = null;
      var chartStress = null;
      var chartCoping = null;
      ...
      function topSchoolsTable(items){
        items = Array.isArray(items) ? items : [];
        if (!items.length) return '<div class="text-muted small">No data</div>';
        var html = '<div class="table-responsive"><table class="table table-sm align-middle mb-0">';
        html += '<thead><tr><th>School</th><th class="text-end">Count</th></tr></thead><tbody>';
        items.forEach(function(it){
          html += '<tr><td>' + esc(it.school) + '</td><td class="text-end fw-semibold">' + esc(it.count) + '</td></tr>';
        });
        html += '</tbody></table></div>';
        return html;
      }
      ...
      function renderDoughnut(canvasId, title, obj, options){
        ...
        return new Chart(el, {
          ...
          data: {
            ...
            datasets: [{
              data: pairs.values,
              backgroundColor: chartColors(pairs.values.length),
              borderWidth: 0,
            }]
          },
          options: {
      ...
      function renderBar(canvasId, title, labels, values, color){
        if (!window.Chart) return null;
        var el = document.getElementById(canvasId);
        if (!el) return null;
        ...
        return new Chart(el, {
          ...
          data: {
            ...
            datasets: [{
              label: title || '',
              data: values,
              backgroundColor: color || 'rgba(13,110,253,0.75)',
              borderRadius: 8,
              borderSkipped: false,
            }]
          },
      ...
      (function(){
        ...
        levelEl.addEventListener('change', function(){ rebuildSchoolOptions(); });
        districtEl.addEventListener('change', function(){ rebuildSchoolOptions(); });
      ...
      async function load(){
        setText('kpiPatients', '...');
        setText('kpiMedicalChecked', '...');
        setText('kpiMedicalPending', '...');
        setText('kpiWithAssessment', '...');

        setHtml('tblBmi', '<div class="text-muted small">Loading...</div>');
        setHtml('tblBp', '<div class="text-muted small">Loading...</div>');
        setHtml('tblStress', '<div class="text-muted small">Loading...</div>');
        setHtml('tblCoping', '<div class="text-muted small">Loading...</div>');
        setHtml('tblTopSchools', '<div class="text-muted small">Loading...</div>');

        try {
          var url = apiUrlWithParams(<?= json_encode(url('/api/medical_stats.php')) ?>, params);
          ...
          setText('kpiMedicalChecked', totals.medical_checked ?? '-');
          setText('kpiMedicalPending', totals.medical_pending ?? '-');
          setText('kpiWithAssessment', totals.with_assessment ?? '-');

          var b = (json.breakdowns || {});
          setHtml('tblBmi', kvTable(b.bmi_category || {}));
          setHtml('tblBp', kvTable(b.bp_category || {}, {
            order: ['normal', 'elevated', 'stage1', 'stage2', 'crisis', 'unknown'],
            labels: {
              normal: 'Normal (<120 / <80)',
              elevated: 'Elevated (120-129 / <80)',
              stage1: 'Stage 1 (130-139 or 80-89)',
              stage2: 'Stage 2 (>=140 or >=90)',
              crisis: 'Crisis (>=180 or >=120)',
              unknown: 'Unknown',
            }
          }));

          setHtml('tblStress', kvTable(b.stress_level || {}, {
            order: ['1', '2', '3', '4'],
            labels: { '1': '1 - Not stressed', '2': '2 - Mild', '3': '3 - Moderate', '4': '4 - High' }
          }));

          setHtml('tblCoping', kvTable(b.coping_level || {}, {
            order: ['1', '2', '3', '4'],
            labels: { '1': '1 - Poor', '2': '2 - Fair', '3': '3 - Good', '4': '4 - Excellent' }
          }));

          setHtml('tblTopSchools', topSchoolsTable(b.top_schools || []));

          destroyChart(chartBmi);
          destroyChart(chartBp);
          destroyChart(chartStress);
          destroyChart(chartCoping);
          destroyChart(chartTopSchools);

          chartBmi = renderDoughnut('chartBmi', '', (b.bmi_category || {}));

          chartBp = renderDoughnut('chartBp', '', (b.bp_category || {}), {
            order: ['normal', 'elevated', 'stage1', 'stage2', 'crisis', 'unknown'],
            labels: {
              normal: 'Normal',
              elevated: 'Elevated',
          ...
          var stressPairs = objToPairs((b.stress_level || {}), {
            order: ['1','2','3','4'],
            labels: { '1': 'Not stressed', '2': 'Mild', '3': 'Moderate', '4': 'High' }
          });
          chartStress = renderBar('chartStress', '', stressPairs.labels, stressPairs.values, 'rgba(220,53,69,0.75)');

          var copingPairs = objToPairs((b.coping_level || {}), {
            order: ['1','2','3','4'],
            labels: { '1': 'Poor', '2': 'Fair', '3': 'Good', '4': 'Excellent' }
          });
          chartCoping = renderBar('chartCoping', '', copingPairs.labels, copingPairs.values, 'rgba(25,135,84,0.75)');

          var schools = Array.isArray(b.top_schools) ? b.top_schools : [];
          var schoolLabels = schools.map(function(x){ return x.school; });
          var schoolVals = schools.map(function(x){ return Number(x.count || 0); });
          chartTopSchools = renderBar('chartTopSchools', '', schoolLabels, schoolVals, 'rgba(13,110,253,0.75)');

        } catch (e) {
          destroyChart(chartBmi); chartBmi = null;
          destroyChart(chartBp); chartBp = null;
          destroyChart(chartStress); chartStress = null;
          destroyChart(chartCoping); chartCoping = null;
          ...
          setHtml('tblBmi', msg);
          setHtml('tblBp', msg);
          setHtml('tblStress', msg);
          setHtml('tblCoping', msg);
